...
class NavigationReader {
  static Future<EpubNavigation> readNavigation(Archive epubArchive, String contentDirectoryPath, EpubPackage package) async {
    ...
    xml.XmlElement ncxNode = containerDocument.findAllElements("ncx", namespace: ncxNamespace).firstWhere((xml.XmlElement elem) => elem != null, orElse: () => null);
    if (ncxNode == null) {
      throw Exception("EPUB parsing error: TOC file does not contain ncx element.");
    }

    xml.XmlElement headNode = ncxNode.findAllElements("head", namespace: ncxNamespace).firstWhere((xml.XmlElement elem) => elem != null, orElse: () => null);
    if (headNode == null) {
    ...
    xml.XmlElement docTitleNode = ncxNode.findElements("docTitle", namespace: ncxNamespace).firstWhere((xml.XmlElement elem) => elem != null, orElse: () => null);
    ...
    xml.XmlElement navMapNode = ncxNode.findElements("navMap", namespace: ncxNamespace).firstWhere((xml.XmlElement elem) => elem != null, orElse: () => null);
    if (navMapNode == null) {
    ...
    xml.XmlElement pageListNode = ncxNode.findElements("pageList", namespace: ncxNamespace).firstWhere((xml.XmlElement elem) => elem != null, orElse: () => null);
  ...
  static EpubNavigationLabel readNavigationLabel(xml.XmlElement navigationLabelNode) {
    ...
    xml.XmlElement navigationLabelTextNode = navigationLabelNode.findElements("text", namespace: navigationLabelNode.name.namespaceUri).firstWhere((xml.XmlElement elem) => elem != null, orElse: () => null);
